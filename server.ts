...
let productQAs: ProductQA[] = [...MOCK_PRODUCT_QA];
let adminReports: AdminReport[] = [...MOCK_ADMIN_REPORTS];
...
async function startServer() {
  ...
  // GET /api/qas
  app.get('/api/qas', (req, res) => {
    res.json({ success: true, qas: productQAs });
  });
  ...
  app.get('/api/products/:id', (req, res) => {
    ...
    const prodQAs = productQAs.filter(q => q.productId === product.id);
  ...
  app.post('/api/reviews/:id/timeline', (req, res) => {
    try {
...
- reviewAuthenticityScore`;
        ...
        if (response.text) {
          ...
          product.aiSummary = summaryData.aiSummary;
          product.pros = summaryData.pros;
          product.cons = summaryData.cons;
          product.reviewQualityScore = summaryData.reviewQualityScore;
          product.verifiedPurchaseScore = summaryData.verifiedPurchaseScore;
          product.communityTrustScore = summaryData.communityTrustScore;
          product.reviewAuthenticityScore = summaryData.reviewAuthenticityScore;
        }
      }
...
Answer this consumer's question accurately, honestly, and directly in 2-4 sentences: "${question}"`;
  ...
  app.post('/api/admin/reports/:id/resolve', (req, res) => {
    const report = adminReports.find(r => r.id === req.params.id);
    if (!report) {
      return res.status(404).json({ success: false, error: 'Report not found' });
    }

    report.status = req.body.action === 'dismiss' ? 'dismissed' : 'resolved';
    if (req.body.action === 'delete_review') {
      reviews = reviews.filter(r => r.id !== report.reviewId);
    }
  ...
  app.put('/api/admin/products/:id/affiliate-links', (req, res) => {
    try {
      const product = products.find(p => p.id === req.params.id);
      if (!product) {
