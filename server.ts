...
// Prevent "Cannot use 'undefined' as a Firestore value" errors when optional
// fields (e.g. videoUrl) are omitted from a form submission
...
// Initialize Bird (formerly MessageBird) email client
const getBirdClient = () => {
  const apiKey = process.env.BIRD_API_KEY;
  if (!apiKey) {
    console.warn('BIRD_API_KEY environment variable is missing.');
    return null;
  }
  return new BirdClient({ apiKey });
};

// Basic HTML escaping so user-submitted contact form text can't break out
// of the email markup or inject arbitrary HTML
...
async function startServer() {
  ...
  app.post('/api/reviews/:id/helpful', async (req, res) => {
    try {
      ...
      const updatedDoc = await reviewsCollection.doc(review.id).get();
      const helpfulCount = (updatedDoc.data() as Review).helpfulCount;
...
Answer this consumer's question accurately, honestly, and directly in 2-4 sentences: "${question}"`;
  ...
  // POST /api/contact (Footer "Contact Us" modal submission, delivered via Bird email API)
  app.post('/api/contact', async (req, res) => {
    try {
      ...
      const { data, error } = await bird.email
        .send({
          from: process.env.BIRD_FROM_EMAIL || 'onboarding@messagebird.dev',
          to: ['everygist2@gmail.com'],
          replyTo: email,
      ...
      if (error) {
        console.error('Bird email send error:', error);
